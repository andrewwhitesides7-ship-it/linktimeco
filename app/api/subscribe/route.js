...
export async function POST(req) {
  try {
    ...
    // Save the contact to your Resend audience (optional, skipped if no ID set)
    ...
    const { error } = await resend.emails.send({
      from: "AJ at Links Time Co <hello@linkstimeco.com>",
    ...
    if (error) {
      console.error("Subscribe send failed:", error.message || error);
      return Response.json(
